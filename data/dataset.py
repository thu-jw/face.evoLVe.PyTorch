...
class FaceDataLoader(object):
    def __init__(self, path_imgrec, data_shape = (3, 112, 112), batch_size=64, transform=None):
        self.transform = transform
        self.faceiter = FaceImageIter(
        ...
        self.batch_size = batch_size
        self.classes = self.faceiter.id2range.keys()

    def __iter__(self):
        return self

    def __next__(self):
        batch = self.faceiter.next()
        batch_data = batch.data[0].asnumpy() # maybe failer
        if self.transform:
    ...
    def __len__(self):
        return len(self.faceiter) // self.batch_size
